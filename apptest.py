    ...
    #resultswb = openpyxl.Workbook()
        ...
        for i,e in enumerate(os.listdir(r"K:\BF\OFS\Bursar\Processing\_External Payments\Scholarships\Queries\Enrollment Queries" + "\\"+ str(fmax))):
            try:
                ...
                continue #continues through the list if there is an unexcpected file type, such as '.db'
        ...
        nodb = []
        for i,e in enumerate(os.listdir(r"K:\BF\OFS\Bursar\Processing\_External Payments\Scholarships\Queries\Exceptions")):
            if e.endswith(".db") == False: #checking each item in dir to see if name ends with '.db' 
                nodb.append(e) 
        ...
        err = traceback.format_exc() #saves traceback as string to varible
        tk.messagebox.showerror(title='ERROR', message= err) #pops up new window displaying the trackback
